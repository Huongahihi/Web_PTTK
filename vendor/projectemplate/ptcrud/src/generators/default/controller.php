    ...
    {   
        ...
        if($request->isAjax){
            ...
            return [
                    ...
                    'footer'=>Html::a('update',['update','<?= substr($actionParams,1) ?>'=><?= $actionParams ?>],['class'=>'btn btn-primary pull-left','role'=>$this->view->params['displayType']]).
            ...
            */
            if ($model->load($request->post())) {
                ...
                <?php if(count($check)!=0): ?>
                    ...
                    if ($<?= $field ?>_file) {
                        $model-><?= $field ?> = FHtml::createUploadFileName($model, $<?= $field ?>_file, '<?= $lower_name ?>', '<?= $field ?>');
                    }
                    <?php endforeach; ?>
                <?php endif; ?>
            ...
            <?php if(count($check)!=0): ?>
                <?php foreach ($check as $field) :?>
                    ...
                    if ($<?= $field ?>_file) {
                        ...
                        $<?= $field ?>_file = $_REQUEST['<?= StringHelper::basename($generator->modelClass) ?>']['<?= $field ?>_file'];
                        $model-><?= $field ?> = $<?= $field ?>_file;
                    }
                <?php endforeach; ?>
            <?php endif; ?>
            <?php if((count($check_datetime)!=0)){ ?>
