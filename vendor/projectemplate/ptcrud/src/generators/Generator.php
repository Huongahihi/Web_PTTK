...
class Generator extends \yii\gii\Generator
{
    ...
    public function generate()
    {
        ...
        //$this->viewPath = '@' . $this->baseFolder . '/web/upload/' . str_replace('-controller', '', Inflector::camel2id( end(explode('\\', $this->controllerClass))));
        //$viewPath = $this->getViewPath();
        //$files[] = new CodeFile("$viewPath/dummy.txt", $this->render('dummy.txt'));
    ...
    public function generateActiveFieldAdvanced($attribute)
    {
        ...
        if ($column->dbType === 'tinyint(1)') {
            return "\$form->field(\$model, '$attribute')->boolean()";
...
//                }
            ...
            if (is_array($column->enumValues) && count($column->enumValues) > 0) {
                ...
                . preg_replace("/\n\s*/", ' ', VarDumper::export($dropDownOptions)) . ", ['prompt' => ''])";
                    ...
                    if($column->type == 'date'){
                        return "\$form->field(\$model,'$attribute')->date()";
                    }
                    elseif($column->type == 'time'){
                        return "\$form->field(\$model,'$attribute')->time()";
                    }
