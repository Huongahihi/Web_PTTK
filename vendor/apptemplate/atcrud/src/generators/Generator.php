<?php
namespace apptemplate\atcrud\generators;
...
class Generator extends \yii\gii\Generator
{
    ...
    public function attributeLabels()
    {
        return array_merge(parent::attributeLabels(), [
            ...
            'modelClass' => 'Array of Tables or Model Classs (seperated by ,): product,article..',
    ...
    public function getModelClassArray() {
        $arrays = explode(',', $this->modelClass);
    ...
    public function generate()
    {
        ...
        foreach ($this->modelClassArrays as $modelClassItem) {
            ...
            if (FHtml::isInArray(str_replace('-', '_', Inflector::camel2id($model)), FHtml::TABLES_COMMON))
                $moduleFolder = '';
...
//            }
            ...
            $viewPath = $this->getViewPath();
            $files[] = new CodeFile("$viewPath/.gitignore", $this->render('.gitignore'));
        }
    ...
    public function generateActiveFieldNoLabel($attribute)
    {
        $input = self::generateActiveField($attribute, '', "\$form->fieldNoLabel");
     ...
     * @return string
     */
    public function generateActiveField($attribute, $colSpan = '', $function = "\$form->field")
    {
        ...
        elseif ($editor == 'text' || $column->dbType === 'text') {
            if (FHtml::isInArray($column->name, $htmlAttributes)) {
        ...
        } elseif ($editor == 'date' || (strpos($column->dbType, 'varchar') !== false && $column->size == 11) || (empty($editor) && (FHtml::isInArray($column->name, $dateAttributes) || StringHelper::startsWith($column->dbType, 'date')))) {
            //Date
