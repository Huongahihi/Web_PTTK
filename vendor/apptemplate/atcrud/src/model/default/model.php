...
?>
    ...
    public $order_by = '<?php foreach ($labels as $name => $label) {
        if (FHtml::isInArray($name, ['sort_order']))
            echo $name . " asc,"; }?><?php foreach ($labels as $name => $label) {
    if (FHtml::isInArray($name, ['is_promotion','is_top','is_hot','is_active','created_date']))
        echo $name . " desc,"; }?>';

    const OBJECTS_RELATED = [<?php foreach ($related_objects as $object) { echo "'" . $object . "', "; }?>];
    const OBJECTS_META = [<?php foreach ($meta_objects as $object) { echo "'" . $object . "', "; }?>];
