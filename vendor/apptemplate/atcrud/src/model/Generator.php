...
namespace apptemplate\atcrud\model;
 ...
 * @since 2.0
 */
class Generator extends \apptemplate\atcrud\Generator
{
    ...
    public function rules()
    {
        return array_merge(parent::rules(), [
            ...
            [['db', 'ns', 'tableName', 'baseClass', 'queryBaseClass'], 'required'],
    ...
    public function attributeLabels()
    {
        return array_merge(parent::attributeLabels(), [
            ...
            'tableName' => 'Array of Tables (seperated by ,): product,article..',
    ...
    public function generate()
    {
        ...
        foreach ($this->getTableNames() as $tableName) {
            ...
            if (empty($this->queryNs))
                $this->queryNs = $this->ns;

            //For Search Model
            $this->modelClass = $this->ns . '\\' . $modelClassName;
            $this->searchModelClass = $modelClassName;

            //echo $tableName . ' ' . $modelClassName . ' ' . $queryClassName . ' '  . '\n';
            $params = [
                'tableName' => $tableName,
            ...
            $files[] = new CodeFile(
                Yii::getAlias('@' . str_replace('\\', '/', $this->ns)) . '/' . $modelClassName . 'Base.php',
            ...
            $files[] = new CodeFile(
                Yii::getAlias('@' . str_replace('\\', '/', $this->ns)) . '/' . $modelClassName . 'API.php',
                $this->render('model_api.php', $params)
            );

            $files[] = new CodeFile(
                str_replace('/models','/messages/en',Yii::getAlias('@' . str_replace('\\', '/', $this->ns))) . '/' . $modelClassName . '.php',
                $this->render('model_labels.php', $params)
            );

            $files[] = new CodeFile(
                str_replace('/models','/messages/vi',Yii::getAlias('@' . str_replace('\\', '/', $this->ns))) . '/' . $modelClassName . '.php',
                $this->render('model_labels.php', $params)
            );

            $files[] = new CodeFile(
                Yii::getAlias('@' . str_replace('\\', '/', $this->ns)) . '/' . $modelClassName . 'Search.php',
                $this->render('search.php', $params)
            );

            $files[] = new CodeFile(
                Yii::getAlias('@backend/..') . '/setup/' . $tableName . '.sql',
                $this->render('model_sql.php', $params)
            );
    ...
    {
        foreach ($this->tableNameArray as $tableNameArrayItem) {
            $this->modelClass = BaseInflector::camel2words($tableNameArrayItem);
            //$class = 'backend\\models\\' . $this->modelClass;
            //var_dump($this->modelClass); $pk = $class::primaryKey(); var_dump($pk); die;
            if ($this->isReservedKeyword($this->modelClass)) {
                $this->addError('modelClass', 'Class name cannot be a reserved PHP keyword.');
            }
            if ((empty($tableNameArrayItem) || substr_compare($tableNameArrayItem, '*', -1, 1)) && $this->modelClass == '') {
...
//        }
    ...
    protected function getTableNames()
    {
        ...
        if (!isset($this->tableNameArray)) {
            $arrays = explode(',', $this->tableName);
            $this->tableNameArray = [];
            foreach ($arrays as $item) {
                $item = trim($item);
                $this->tableNameArray[] = $item;
            }
        }

        $tableNames = [];
        foreach ($this->tableNameArray as $tableNameItem) {
            if (strpos($tableNameItem, '*') !== false) {
                if (($pos = strrpos($tableNameItem, '.')) !== false) {
                    $schema = substr($tableNameItem, 0, $pos);
                    $pattern = '/^' . str_replace('*', '\w+', substr($tableNameItem, $pos + 1)) . '$/';
                } else {
                    $schema = '';
                    $pattern = '/^' . str_replace('*', '\w+', $tableNameItem) . '$/';
                }
            ...
            } elseif (($table = $db->getTableSchema($tableNameItem, true)) !== null) {
                $tableNames[] = $tableNameItem;
                if (empty($this->modelClass))
                    $this->modelClass = $this->generateClassName($tableNameItem);//str_replace(' ', '', BaseInflector::camel2words($this->tableName));

                $this->classNames[$tableNameItem] = $this->modelClass;
            }
        }
...
//        }
        ...
        $this->classNames[$fullTableName] = $className;
        return $className;
    }
     ...
     */
    public function getColumnNames($tablSchema = null)
    {
        try {
            ...
            if (!isset($tablSchema))
                $tablSchema = $class::getTableSchema();

            if (is_subclass_of($class, 'yii\db\ActiveRecord')) {

                return $tablSchema->getColumnNames();
            } else {
                return $tablSchema->getColumnNames();
     ...
     * @return array the generated validation rules
     */
    public function generateSearchRules($table = null)
    {
        if (!isset($table))
     ...
     * @return array
     */
    public function generateSearchConditions($searchExact = false, $table = null)
    {
        if (!isset($table))
            $table = $this->getTableSchema();
