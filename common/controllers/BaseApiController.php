...
class BaseApiController extends Controller
{
    ...
    {
        $action1 = FHtml::getApplicationNamespace() . '\\actions\\' . $id . 'Action';
        $action2 = FHtml::getApplicationNamespace() . '\\backend\\modules\\' . FHtml::currentModule() . '\\actions\\' . $id . 'Action';
        $actions = [$action1, $action2];
        foreach ($actions as $action) {
            if (class_exists($action)) {
                return \Yii::createObject($action, [$id, $this]);
            }
        }

        $controller_Class = FHtml::getApplicationNamespace() . '\\backend\\modules\\' . FHtml::currentModule() . '\\controllers\\' . BaseInflector::camelize($this->id) . 'Controller';
        if (class_exists($controller_Class)) {
            $controller = \Yii::createObject($controller_Class, [$this->id, $this]);
    ...
    public function actionSettings()
    {
        ...
        $result = FHtml::getArray($key, $object_type, $column, false);
        $out = FHtml::getOutputForAPI($result, $key,  '', 'data', 1);
        $out['code'] = $key;
        return $out;
    }
}
