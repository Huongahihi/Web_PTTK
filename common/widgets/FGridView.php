...
class FGridView extends GridView
{
    ...
    public function run()
    {
        ...
        if (isset($listView)) {
            ...
            if (!empty($this->dataProvider->getTotalCount()))
                parent::run();
            else {
                echo FHtml::showEmptyMessage();
            }
        }
    }
            ...
            if ($this->display_type != FHtml::DISPLAY_TYPE_WIDGET) {
                ...
                $cells[] = [
                    ...
                    'urlCreator' => function ($action, $model) {
                        ...
                        //$controller = str_replace(FHtml::getTableName($model), '_', '-');
                         //return FHtml::createUrl("$module/$controller/$action", ['id' => $model->id, 'form_type' => FHtml::getRequestParam('form_type')]);
...
//                $column->groupedRow = false;
            ...
            if (FHtml::field_exists($column, 'attribute') && $column->attribute == 'id') {
                ...
                if (FConfig::settingShowPreviewColumn() == true) {
                    $this->columns[$i] = $column;
                } else {
        ...
        if ($this->isFormEnabled()) {
            $form_view = !empty($this->form_view) ? $this->form_view : '_add';
            $viewFile = FHtml::findViewFile( $form_view);
            if (is_file($viewFile)) {
                $this->form_view = $form_view;
                $this->form_field_template = '{object}[{column}]';
                FHtml::registerPlusJS($this->object_type, $columns, $this->id, $this->form_field_template, $this->default_fields);
            } else if (is_string($this->form_fields) && !empty($this->form_fields)) {
    ...
    public function renderFilters()
    {
        ...
        if (($this->isFormEnabled() && empty($this->form_fields)) && $this->filterEnabled === true && !isset($this->filterModel) && empty($this->form_view)) {
    ...
    protected function renderToolbar($populateAll = false)
    {
        ...
        if ($this->isFormEnabled() && !empty($this->form_view)) {
            ...
            $result1 = FHtml::render($this->form_view, '', ['model' => FHtml::createModel($this->object_type), 'name' => 'Quick_Add'], FHtml::currentModule(), false);

            if (!empty($result1))
                $result .= "<div class='hidden-print clear-both' style='border: 1px solid lightgray; padding: 10px'>$result1</div>";
        }
    ...
    public function renderToggleData()
    {
        ...
        $label = $form_enabled ? '<i class="fa fa-undo"></i>' : '<span class="glyphicon glyphicon-plus"></span>';
