...
class FListView extends ListView
{
        ...
        if (empty($this->display_type) || $this->display_type == self::VIEW_DEFAULT) //default view
        {
            ...
            if (empty($this->itemView))
                $this->itemView = function ($model, $key, $index, $widget) {
                    $result = "<div class='row' style='padding-top:15px; padding-bottom:15px;border-bottom: 1px lightgray dashed'><div class='col-md-1 col-xs-3'>" . FHtml::showImage($model) . "</div><div class='col-md-10 col-xs-9'>" . FHtml::showModel($model, $this->field_name, $this->field_description, $this->itemLayout) . '<br/>' . FHtml::showModel($model, '', $this->field_business, FHtml::LAYOUT_TEXT) . '<br/>'. FHtml::showModel($model, [], $this->field_group, $this->itemLayout) . "</div><div class='col-md-1 col-xs-12'><div class='pull-right'>" . self::renderDataCellContent($model, null, null) . "</div></div></div>";
                    return $result;
                };
            $this->layout = "<h1>fsdfs</h1>";
            ...
            if (empty($this->itemView))
                $this->itemView = function ($model, $key, $index, $widget) {
                    return "<tr><td class='col-xs-2'>" . FHtml::showImage($model) . "</td><td class='col-xs-4'>" . FHtml::showModel($model, $this->field_name, $this->field_description, $this->itemLayout) . "</td><td class='col-xs-3'>" . FHtml::showModel($model, '', $this->field_business, FHtml::LAYOUT_TEXT) . "</td><td class='col-xs-2'>" . FHtml::showModel($model, [], $this->field_group, $this->itemLayout) . "</td></tr>";
                };
    ...
    {
        return "<div class='row clear-both'>" . "<div style='padding:10px'>" . FHtml::showEmptyMessage() . "</div></div>";
