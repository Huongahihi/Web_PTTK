...
class FModel extends FConfig
{
    ...
    public static function findAll($table, $condition = [], $order_by = [], $page_size = -1, $page = -1, $load_active_only = true, $selected_fields = []) {
        if (is_object($table))
            $table = FHtml::getTableName($table);

        return self::getModels($table, $condition, $order_by, $page_size, $page, false, $load_active_only, $selected_fields);
    }
    ...
    public static function getModel($table, $type = '', $id = null, $default_fields = null, $autoCreateNew = true)
    {
        ...
        if (strpos($table, '\\') !== false) {
            if (class_exists($table)) {
                $model = Yii::createObject(['class' => $table::className()]);
            }
        } else {
            if (is_array($type)) {
                $default_fields = $type;
            ...
            $className2 = empty($currentModule) ? '' : FHtml::getApplicationNamespace() . '\\backend\\modules\\' . FHtml::currentModule() . '\\models\\' . $baseClassName;
    ...
    public static function field_exists($model, $field)
    {
        ...
        try {
            if ((is_object($model) && property_exists($model, $field))
                ...
                || (self::is_numeric($field) && is_array($model) && count($model) > $field)
    ...
    public static function getModelAPI($model)
    {
        if (is_string($model))
        {
            ...
            $className = "backend\\modules\\$module\\models\\{$model}API";
        }
        else if (is_object($model)) {
            $className = $model::className() . 'API';
        } else {
            return null;
        }
        if (class_exists($className)) {
...
//                    }
    ...
    public static function getArray($key, $table = '', $column = '', $isCache = false, $select = '', $search_params = [], $limit = 0)
    {
        ...
        } else if (($key == 'app_user' || $key == '@user') & $column == 'user_id') {
            $table = 'app_user';
            $key = '@user';
            $data = [];
    ...
    public static function getModels($object_type, $search_params = [], $order_by = [], $page_size = -1, $page = 1, $isCached = false, $load_active_only = true, $selectedFields = [])
    {
        ...
        $models = isset($list) ? $list->getModels() : null;
    ...
    public static function getPageModelsList($object_type = '', $search_params = [], $order_by = [], $page_size = -1, $page = 1, $isCached = false, $display_fields = [])
    {
        ...
        return self::getModelsList($object_type, $search_params, $order_by, $page_size, $page, $isCached, true, $display_fields);
    }
...
//        }
    ...
    public static function currentDb($db_config = '')
    {
        if (empty($db_config))
            ...
            $arr = [$db_config, FHtml::currentApplicationDatabase(), FHtml::CONFIG_DB];
    ...
    public static function setModelCustomAttribute($object_type, $object_id = '', $field_name = '', $field_value = null) {
        ...
        $model = FHtml::getModel('object_attributes', ['object_type' => $object_type, 'object_id' => $object_id, 'meta_key' => $field_name]);
        $result = false;
        if (isset($model)) {
    ...
    public static function saveUploadedFiles($model, $files = [], $folder = '', $baseFolder = '') {
        ...
        if (!empty($upload_fields)) {
            ...
            $files = empty($files) ? FHtml::getUploadedFiles($model, $upload_fields, $folder . FHtml::getAttribute($model, 'id')) : $files;
        ...
        if (is_array($arrays) && !empty($arrays)) {
            ...
            foreach ($arrays as $array) {
                ...
                if (empty($id)) {
                    ...
                    if (self::field_exists($item, 'file_type') && key_exists('file_type', $array))
                    ...
                    if (empty($item->title)) {
                        if (isset($file))
                            $item->title = BaseInflector::camel2words($file->name);
                        else
                    ...
                    if (isset($item)) {
                        ...
                        if (empty($item->title) && isset($file))
                            $item->title = BaseInflector::camel2words($file->name);
    ...
    public static function getFileType($file) {
        ...
        if (in_array($file_extension, ['jpg', 'png', 'gif']))
            ...
            return 'Audio';
        else if (in_array($file_extension, ['mp4', 'mov']))
            return 'Video';
        else
            return 'File';
...
//        echo $object2_module; echo $object_module;
//        var_dump($_POST); die;
        //echo $object_module; echo $object2_module;
        if (empty($arrays)) {
            if (key_exists($object_module, $_POST) && key_exists($object2_module, $_POST[$object_module])) {
                ...
                if (is_array($arrays) && !empty($arrays)) {
                    ...
                    foreach ($arrays as $array) {
                        ...
                        $item->created_user = FHtml::currentUserId();
                        $item->object2_type = $object2_type;
                        $item->object2_id = (!is_array($array)) ? $array : FHtml::getFieldValue($array, ['id', 'name']);
                        $item->relation_type = $relation_type;
                        $item->save();
                    }
                }
