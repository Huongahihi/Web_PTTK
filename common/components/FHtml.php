...
class FHtml extends FApi
{
    ...
    public static function buttonCreate($icon = '<i class="glyphicon glyphicon-plus"></i>', $title = 'Create', $action = 'create', $color = 'success', $style =  'float:left; margin-right:10px;' ) {
        $button = self::buttonAction($icon, $title, $action, $color, false, $style);
    ...
    public static function buttonAction($icon = '', $title = '', $action = '', $color = '', $confirm = false, $style = 'float:left; margin-right:10px;') {
    ...
    public static function showImage($image, $model_dir = '', $width = '100%', $height = 0, $css = '', $title = '', $show_empty_image = TRUE, $hover_effect = '', $linkurl = '')
    {
        ...
        if (strlen($image) > 0) {
            if (StringHelper::endsWith($image, '.mp3')) {
                return FHtml::defaultImage($show_empty_image, $width, $height, $model_dir);
...
//                ]);
            ...
            if (strpos($image, 'api.soundcloud.com') !== false) {
                ...
                $baseUpload = FHtml::getFullUploadFolder($model_dir);

                if (StringHelper::startsWith($image, '/') || (!empty($model_dir) && strpos($baseUpload, $model_dir)) !== false) {
...
//                            ['alt' => $title, 'class' => $css]);
        ...
        if (empty($str)) {
            if (strlen($imageLink) > 0) {
                ...
                if (FHtml::is_image($imageLink)) {
                    ...
                    $str =  "<a target='_blank' data-pjax=0 alt='$image - $model_dir - $imageLink' href='$imageLink' class='btn btn-xs btn-sm'><span class='glyphicon glyphicon-file'></span> </a>";
                }
            }
        }
    ...
    public static function getFileURL($image, $model_dir = '', $position = false, $default_file = \Globals::NO_IMAGE)
    {
        ...
        if (filter_var($image, FILTER_VALIDATE_URL)) {
            ...
            if ($model_dir === 'www') { // common folder for all applications
                $applicationid = FHtml::currentApplicationId();

                $base_url = self::getBaseUrl(null, true);

                $image_path = $base_url . "/applications/$applicationid/upload/www/$image";

            } else if (StringHelper::startsWith($model_dir, 'www/')) { // common folder for all applications
                $baseUpload = Yii::getAlias('@' . UPLOAD_DIR);
            ...
            }  else if (empty($model_dir) || strpos($model_dir, '/') !== false)  {
                $image_path =  FHtml::currentBaseURL(). $model_dir . '/' . $image;
            } else {
        ...
        if (!StringHelper::startsWith($image_path, 'http'))
            $image_path      = FHtml::currentHost() . $image_path;
    ...
    public static function getImageUrl($image, $model_dir = false, $position = false)
    {
        ...
        foreach ($images as $image) {
            $result = self::getFileURL($image, $model_dir, $position);
    ...
    {
        $name = self::cleanStringVietnamese($name);
        $name = self::str_replace($name, ['--' => '-']);
        return strtolower( $name);
    }

    public static function str_replace($name, $params = [])
    {
...
//            return null;
    ...
    public static function showPrice($value, $old_value = '', $discount = '', $prefix = '$', $color = 'red', $show_friendly = true, $show_price = true)
    {
        ...
        if (!empty($old_value) && !empty($show_friendly))
            $result .= '<span class = "price-old line-through text-default">' . self::showCurrency($old_value, $prefix) . '</span>';

        if (!empty($discount) && !empty($show_friendly)) {
            if (is_numeric($discount))
                $discount = '-' . $discount . '%';
            $result .= '<small class = "price-discount">' . $discount . '</small>';
        }
        return $result;
    }
...
// overflow
    ...
    public static function getUploadFolder($model) {
...
//                $result = FHtml::NULL_VALUE;
    ...
    public static function showObjectPreview($model, $id_field = 'id', $object_type_field = '') {
        ...
        $image = FHtml::showImage($image, FHtml::getImageFolder($model), '100%', '');
        ...
        return "<div class='row'><div class='col-md-2'>$image</div><div class='col-md-10'><b>$name</b> <br/>$description.<br/> <small>$status</small></div></div>";
    }
    ...
    public static function timeAgo($time_ago)
    {
        ...
        // Seconds
        if($seconds <= 60){
            return Fhtml::t('common','Just now') ;
        }
        //Minutes
        else if($minutes <=60){
            if($minutes==1){
                return FHtml::t('common','One minute ago');
            }
            else{
                return $minutes .FHtml::t('common','minutes ago');
            }
        }
        //Hours
        else if($hours <=24){
            if($hours==1){
                return FHtml::t('common','An hour ago');
            }else{
                return $hours .FHtml::t('common','hrs ago');
            }
        }
        //Days
        else if($days <= 7){
            if($days==1){
                return FHtml::t('common','Yesterday');
            }else{
                return $days .FHtml::t('common','days ago');
            }
        }
        //Weeks
        else if($weeks <= 4.3){
            if($weeks==1){
                return FHtml::t('common','A week ago');
            }else{
                return $weeks .FHtml::t('common','weeks ago');
            }
        }
        //Months
        else if($months <=12){
            if($months==1){
                return FHtml::t('common','A month ago');
            }else{
                return $months .FHtml::t('common','months ago');
            }
        }
        //Years
        else{
            if($years==1){
                return FHtml::t('common', 'One year ago');
            }else{
                return $years .FHtml::t('common','years ago');
            }
        }
    }
    ...
    public static function getApplicationUploadFolder($folder, $position = false) {
        // if folder is www (logo, background..) then take from application folder directly
        $application_folder = FHtml::currentApplicationFolder();
        //$application_id = FHtml::currentApplicationId();

//        if (strpos($folder, 'www'))
//            $application_folder = $application_id;

        if (!empty($application_folder)) {
            $folder = str_replace('backend\\web\\upload', "applications\\$application_folder\\upload", $folder);
            $folder = str_replace('backend/web/upload', "applications/$application_folder/upload", $folder);
        }
...
//                }
    ...
    public static function buttonSave($model = null, $color = 'default') {
        return FHtml::submitButton('<i class="fa fa-save"></i> ' . FHtml::t('common', 'Save'), ['class' => 'btn btn-' . $color, 'onclick' => 'submitForm("save")']);
    }

    public static function buttonSaveBack($model = null) {
        return FHtml::submitButton('<i class="fa fa-save"></i> ' . FHtml::t('common', 'Save & Back'), ['class' => 'btn btn-primary']);
    }

    public static function buttonEdit($model = null) {
        return FHtml::a('<i class="fa fa-edit"></i> ' . FHtml::t('common', 'update'), ['update', 'id' => $model->id], ['class' => 'btn btn-warning pull-right', 'data-pjax' => 0]);
    }
    ...
    public static function buttonView($model = null) {
        return FHtml::a('<i class="fa fa-print"></i> ' . FHtml::t('common', 'Preview'), ['view', 'id' => $model->id], ['class' => 'btn btn-default', 'data-pjax' => 0]);
    }

    public static function buttonCancel($model = null) {
        return FHtml::a('<i class="fa fa-undo"></i> ' . FHtml::t('common', 'Back'), ['index'], ['class' => 'btn btn-default', 'data-pjax' => 0]);
    }

    public static function buttonCloseQuickAdd($model = null) {
        return FHtml::a('<i class="fa fa-undo"></i> ' . FHtml::t('common', 'Close'), ['index', 'form_enabled' => 0], ['class' => 'btn btn-default', 'data-pjax' => 0]);
    }

    public static function buttonDelete($model = null) {
        return FHtml::a('<i class="fa fa-trash"></i> ' . FHtml::t('common', 'Delete'), ['delete', 'id' => $model->id], [
    ...
    public static function getBaseUrl($view = null, $get_from_root = false)
    {
        if (!isset($view)) {
            ...
            if ($get_from_root)
                $result = self::str_replace($result, ['backend/web' => '']);
    ...
    public static function createLink($url, $params = [], $position = BACKEND, $label = '...', $target = '_blank', $css = 'btn btn-xs btn-default')
    {
        ...
        return '<a class="hidden-print" data-pjax=0 href="' . $url . '" target="' . $target . '" class="' . $css . '">' . $label . '</a><span class="visible-print">' . $label . '</span> ';
    }
...
//                            continue;
    ...
    {
        if (FConfig::settingAdminInlineEdit() !== true || $canEdit != true || !FHtml::isAuthorized(FHtml::ACTION_EDIT, $object_type, $field))
            return $content;

        $field_label = FHtml::t('common', $field);
        if (empty($content) || $content === FHtml::NULL_VALUE)
            $content = "<span style='color:lightgrey'>" . $field_label . "</span>"; //FHtml::NULL_VALUE;
        ...
        if (is_array($changeFields)) {
            foreach ($changeFields as $oldField => $newField) {
                if ($isMultipleForm) {
                    ...
                    $populateFields .= "alert($oldField + '-' + $newField); $('#'+ id_" . $oldField . ").val(data['" . $newField . "']);";
                } else {
                    $populateFields .= "alert($oldField + '-' + $newField); $('#" . $controlID . "-" . $oldField . "').val(data['" . $newField . "']);";
                }
            }
    ...
    public static function showPlusButton($object_type, $is_column = true, $is_search = false)
    {
        ...
        if ($is_search == true) {
            ...
            $result = '<a class="btn btn-success" href="#" onclick="plus' . BaseInflector::camelize($object_type) . '()" value="+"> <span class="glyphicon glyphicon-plus"></span> ' . $add_t . ' </a>';
        }
        return $result;
    }
    ...
    public static function cleanStringVietnamese($cs, $demiliter = '-', $tolower = false)
    {
        ...
        $marTViet = array(
            ...
            "Ỳ", "Ý", "Ỵ", "Ỷ", "Ỹ",
            "Đ", " ", "_", "-", "--", ",", ":", '"', ".", "'", "&", "%", "*", "@", "!", "#", "$", "^", "(", ")", "/", "\\", ")", "(", "{", "}", "[", "]", "?", "<", ">", "|", "+", "=", "`", "’");
        $marTViet = str_replace(["$demiliter"], "", $marTViet);
    ...
    public static function showArrayAsTable($arr, $layout_type = 'table', $background = '#fafbfc') {
        $str = '';
        if (ArrayHelper::isIndexed($arr)) {
            foreach ($arr as $val) {
                $str .= self::showArrayAsTable($val, $layout_type) . '<br/>';
                if (count($arr) > 1)
                    $str .= '<hr/>';
            }
        } else {
            if ($layout_type == 'table') {
                $str = "<table class='table table-bordered table-condensed' width='100%' border='0px' cellpadding='30' cellspacing='0' style=''><tbody>";
                foreach ($arr as $key => $val) {
                    if (!is_numeric($key)) {
                        $key = FHtml::t('common', ucfirst(FHtml::t('common', $key)));

                        $str .= "<tr>";
                        $str .= "<td class='col-md-3' width='30%' style='padding: 15px;border:1px dashed lightgray; background-color:$background'><b>$key</b></td>";
                        $str .= "<td style='padding: 15px; border:1px dashed lightgray; ' width='70%'; >";
                        if (is_array($val)) {
                            if (!empty($val)) {
                                $str .= self::showArrayAsTable($val, 'line');
                            }
                        } else {
                            $str .= "$val";
                        }
                ...
                $str = '<div style="line-height: 200%; padding-bottom: 10px; margin-bottom: 10px;"> ';
                foreach ($arr as $key => $val) {
                    $str .= '<b>' . ucfirst(FHtml::t('common', $key)) . '</b>: ' . $val . '<br/>';
                }
    ...
    public static function showEmptyMessage($model = null, $text = 'Sorry, there is no data available or we could not find any matches', $action = true) {
        ...
        if (is_bool($action) && FHtml::isRoleAdmin()) {
            $t .= "<div style='text-align:center; padding-bottom:50px;'>" . FHtml::buttonCreate('', 'Create First Record', 'create', 'success', '') . "</div>";
