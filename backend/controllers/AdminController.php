...
class AdminController extends BaseAdminController
{
    ...
    public function actionListLookup()
    {
        ...
        if (!is_null($q) || !empty($q)) {
            $data = FHtml::getArray('@' . $search_object, $search_object, '', false, '', ['like', $search_field, $q]);
            $out['results'] = array_values($data);
        } else if (is_null($q) || empty($q)) {
            $data = FHtml::getArray('@' . $search_object, $search_object, '', false, '', []);
            $out['results'] = array_values($data);
        } elseif ($id > 0) {
    ...
    {
        $table = FModel::getTableName(FModel::currentController());
        FModel::deleteAll($table);
        FHtml::refreshCache();
