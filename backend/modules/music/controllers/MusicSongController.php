...
use backend\modules\music\models\MusicSongSearch;
...
/**
 * MusicSongController implements the CRUD actions for MusicSong model.
 */
class MusicSongController extends AdminController
{
    protected $moduleName = 'MusicSong';
    protected $moduleTitle = 'Music Song';
    protected $moduleKey = 'music_song';
    protected $object_type = 'music_song';
...
/**
    ...
    /**
     * Lists all MusicSong models.
     * @return mixed
     */
    public function actionIndex()
    {    
              $searchModel = new MusicSongSearch();
       ...
       if (Yii::$app->request->post('hasEditable')) {
           ...
           $model = MusicSong::findOne($Id);

           $out = Json::encode(['output' => '', 'message' => '']);

           $post = [];
           $posted = current($_POST['MusicSong']);
           $post['MusicSong'] = $posted;
    ...
    /**
     * Displays a single MusicSong model.
     * @param string $id
     * @return mixed
     */
    ...
    public function actionCreate($type = false)
    {
        ...
        if($request->isAjax){
            ...
            if ($model->load($request->post())) {
                $model->id = null;
                $album_ids = $model->album_id;
                $model->album_id = 0;
                if ($model->save()) {
                    $id = $model->id;
                    $this->saveAlbum($album_ids, $id);
    ...
    /**
     * Updates an existing MusicSong model.
     * For ajax request will return json object
    ...
    public function actionUpdate($id)
    {
        ...
        if (!empty($model->albums)) {
            foreach ($model->albums as $item) {
                $album_ids[] = $item->album->id;
            }
            $model->album_id = $album_ids;
        }
    ...
    /**
     * Delete an existing MusicSong model.
     * For ajax request will return json object
    ...
    public function actionDelete($id)
    {
     ...
     /**
     * Delete multiple existing MusicSong model.
     * For ajax request will return json object
     * and for non-ajax request if deletion is successful, the browser will be redirected to the 'index' page.
     * @param string $id
     * @return mixed
    ...
    /**
     * Finds the MusicSong model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param string $id
     * @return MusicSong the loaded model
    ...
    public function saveAlbum($album_ids, $song_id)
    {
        if (!empty($album_ids)) {
            foreach ($album_ids as $key => $album_id) {
                echo $album_id;
                $check_album = ObjectRelation::find()->where([
                        'object_id'    => $album_id,
                        'object2_id'   => $song_id,
                        'object_type'  => 'music_playlist',
                        'object2_type' => 'music_song',
                    ])->count();
                if ($check_album == 0) {
                    $relation = new ObjectRelation;
                    $relation->object_id     = $album_id;
                    $relation->object_type   = 'music_playlist';
                    $relation->object2_id    = $song_id;
                    $relation->object2_type  = 'music_song';
                    $relation->sort_order    = 1;
                    $relation->created_date  = date("Y-m-d");
                    $relation->created_user  = Yii::$app->user->identity->id;
                    $relation->save();
                }
            }
        }
