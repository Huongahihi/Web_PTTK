...
use backend\modules\app\models\AppUserProSearch;
...
/**
 * AppUserProController implements the CRUD actions for AppUserPro model.
 */
class AppUserProController extends AdminController
{
    protected $moduleName = 'AppUserPro';
    protected $moduleTitle = 'App User Pro';
    protected $moduleKey = 'app_user_pro';
...
/**
    ...
    /**
     * Lists all AppUserPro models.
     * @return mixed
     */
    public function actionIndex()
    {    
        $searchModel = new AppUserProSearch();
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams);
           ...
           if (Yii::$app->request->post('hasEditable')) {
               ...
               $post = [];
               $posted = current($_POST['AppUserPro']);
               $post['AppUserPro'] = $posted;
    ...
    /**
     * Displays a single AppUserPro model.
     * @param integer $id
     * @return mixed
     */
    ...
    /**
     * Creates a new AppUserPro model.
    ...
    public function actionCreate($type = false)
    {
        ...
        $uploadFields = AppUserPro::COLUMNS_UPLOAD;
        ...
        if($request->isAjax){
            ...
            */
            ...
            if($request->isGet){
                ...
                'title'=> FHtml::t($this->moduleName),
                'content'=>'<span class="text-success">Create AppUserPro success</span>',
                'footer'=> Html::button(FHtml::t('Close'),['class'=>'btn btn-default pull-left','data-dismiss'=>"modal"]).
    ...
    /**
     * Updates an existing AppUserPro model.
     * For ajax request will return json object
        ...
        if($request->isAjax){
            ...
            */
            ...
            if ($model->load($request->post())) {
                ...
                $files =FHtml::getUploadedFiles($model, $uploadFields, 'app-user-pro' . FHtml::getAttribute($model, 'user_id'), $oldModel);

                if ($model->save()) {
                    FHtml::saveFiles($files, $this->uploadFolder .  '/app-user-pro/');
                    FHtml::saveModel($modelMeta, $request->post(), ['app_user_pro_id' => $model->id]);
    ...
    /**
     * Delete an existing AppUserPro model.
     * For ajax request will return json object
    ...
    public function actionDelete($id)
    {
     ...
     /**
     * Delete multiple existing AppUserPro model.
     * For ajax request will return json object
     * and for non-ajax request if deletion is successful, the browser will be redirected to the 'index' page.
     * @param integer $id
     * @return mixed
    ...
    /**
     * Finds the AppUserPro model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return AppUserPro the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = AppUserPro::findOne($id)) !== null) {
            return $model;
        } else {
