...
class ApplicationController extends AdminController
{
    ...
    protected $moduleTitle = 'Application';
    protected $moduleKey = 'application';
    protected $object_type = 'application';
    ...
    public function actionIndex()
    {
        if (!APPLICATIONS_ENABLED) {
            ...
            $model = Application::findOne(['code' => $application_id]);
        ...
        $application_ids = FHtml::getUserApplications();

        if (!empty($application_ids) && count($application_ids) < 2) {
        ...
        if (Yii::$app->request->post('hasEditable')) {
            ...
            $post = [];
            $posted = current($_POST['Application']);
            $post['Application'] = $posted;
    ...
    public function actionCreate($type = false)
    {
        ...
        if($request->isAjax){
            ...
            if ($model->load($request->post())) {
                ...
                return $this->render('create', ['model' => $model, 'modelMeta' => null]);
            } else {
                return $this->render('create', ['model' => $model, 'modelMeta' => null]);
            }
        }
    ...
    public function actionUpdate($id)
    {
        ...
        if($request->isAjax){
            ...
            if ($model->load($request->post())) {
                ...
                return $this->render('update', ['model' => $model, 'modelMeta' => null]);
            } else {
                return $this->render('update', ['model' => $model, 'modelMeta' => null]);
            }
        }
    ...
    /**
     * Delete an existing Application model.
     * For ajax request will return json object
    ...
    /**
     * Delete multiple existing Application model.
     * For ajax request will return json object
     * and for non-ajax request if deletion is successful, the browser will be redirected to the 'index' page.
     * @param integer $id
     * @return mixed
