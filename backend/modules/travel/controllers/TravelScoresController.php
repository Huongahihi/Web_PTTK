...
use backend\modules\travel\models\TravelScoresSearch;
...
/**
 * TravelScoresController implements the CRUD actions for TravelScores model.
 */
class TravelScoresController extends AdminController
{
    protected $moduleName = 'TravelScores';
    protected $moduleTitle = 'Travel Scores';
    protected $moduleKey = 'travel_scores';
...
/**
    ...
    /**
     * Lists all TravelScores models.
     * @return mixed
     */
    public function actionIndex()
    {    
        $searchModel = new TravelScoresSearch();
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams);
           ...
           if (Yii::$app->request->post('hasEditable')) {
               ...
               $post = [];
               $posted = current($_POST['TravelScores']);
               $post['TravelScores'] = $posted;
    ...
    /**
     * Displays a single TravelScores model.
     * @param string $id
     * @return mixed
     */
    ...
    /**
     * Creates a new TravelScores model.
    ...
    public function actionCreate($type = false)
    {
        ...
        $uploadFields = TravelScores::COLUMNS_UPLOAD;
        ...
        if($request->isAjax){
            ...
            */
            ...
            if($request->isGet){
                ...
                'title'=> FHtml::t($this->moduleName),
                'content'=>'<span class="text-success">Create TravelScores success</span>',
                'footer'=> Html::button(FHtml::t('Close'),['class'=>'btn btn-default pull-left','data-dismiss'=>"modal"]).
            ...
            */
            ...
            if ($model->load($request->post())) {
                ...
                if ($model->save()) {
                    FHtml::saveModel($modelMeta, $request->post(), ['travel_scores_id' => $model->id]);

                    $files =FHtml::getUploadedFiles($model, $uploadFields, 'travel-scores' . FHtml::getAttribute($model, 'id'), $oldModel);

                    FHtml::saveFiles($files, $this->uploadFolder .  '/travel-scores/', $model);
    ...
    /**
     * Updates an existing TravelScores model.
     * For ajax request will return json object
        ...
        if($request->isAjax){
            ...
            */
            ...
            if ($model->load($request->post())) {
                ...
                if ($model->save()) {
                    FHtml::saveFiles($files, $this->uploadFolder .  '/travel-scores/');
                    FHtml::saveModel($modelMeta, $request->post(), ['travel_scores_id' => $model->id]);
                    FHtml::saveObjectItems($model, $this->moduleKey, $model->id, $model::getRelatedObjects());
                    if ($this->saveType() == 'clone')
                    {
    ...
    /**
     * Delete an existing TravelScores model.
     * For ajax request will return json object
    ...
    public function actionDelete($id)
    {
     ...
     /**
     * Delete multiple existing TravelScores model.
     * For ajax request will return json object
     * and for non-ajax request if deletion is successful, the browser will be redirected to the 'index' page.
     * @param string $id
     * @return mixed
    ...
    /**
     * Finds the TravelScores model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param string $id
     * @return TravelScores the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = TravelScores::findOne($id)) !== null) {
            return $model;
        } else {
