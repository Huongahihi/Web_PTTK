...
use backend\modules\travel\models\TravelPlansSearch;
...
/**
 * TravelPlansController implements the CRUD actions for TravelPlans model.
 */
class TravelPlansController extends TravelAdminController
{
    protected $moduleName = 'TravelPlans';
    protected $moduleTitle = 'Travel Plans';
    protected $moduleKey = 'travel_plans';
    ...
    /**
     * Lists all TravelPlans models.
     * @return mixed
     */
    public function actionIndex()
    {
        $searchModel = new TravelPlansSearch();
        ...
        if (Yii::$app->request->post('hasEditable')) {
            ...
            $post = [];
            $posted = current($_POST['TravelPlans']);
            $post['TravelPlans'] = $posted;
    ...
    /**
     * Displays a single TravelPlans model.
     * @param string $id
     * @return mixed
     */
    ...
    /**
     * Creates a new TravelPlans model.
    ...
    public function actionCreate($type = false)
    {
        ...
        $uploadFields = TravelPlans::COLUMNS_UPLOAD;
        ...
        if ($request->isAjax) {
            ...
            */
            ...
            if ($request->isGet) {
                ...
                return [
                    ...
                    'title' => FHtml::t($this->moduleName),
                    'content' => '<span class="text-success">Create TravelPlans success</span>',
                    'footer' => Html::button(FHtml::t('Close'), ['class' => 'btn btn-default pull-left', 'data-dismiss' => "modal"]) .
            ...
            */
            ...
            if ($model->load($request->post())) {
                ...
                if ($model->save()) {
                    FHtml::saveModel($modelMeta, $request->post(), ['travel_plans_id' => $model->id]);

                    $files = FHtml::getUploadedFiles($model, $uploadFields, 'travel-plans' . FHtml::getAttribute($model, 'id'), $oldModel);

                    FHtml::saveFiles($files, $this->uploadFolder . '/travel-plans/', $model);
    ...
    /**
     * Updates an existing TravelPlans model.
     * For ajax request will return json object
        ...
        if ($request->isAjax) {
            ...
            */
            ...
            if ($model->load($request->post())) {
                ...
                if ($model->save()) {
                    FHtml::saveFiles($files, $this->uploadFolder . '/travel-plans/');
                    FHtml::saveModel($modelMeta, $request->post(), ['travel_plans_id' => $model->id]);
                    FHtml::saveObjectItems($model, $this->moduleKey, $model->id, $model::getRelatedObjects());
                    if ($this->saveType() == 'clone') {
    ...
    /**
     * Delete an existing TravelPlans model.
     * For ajax request will return json object
    ...
    /**
     * Delete multiple existing TravelPlans model.
     * For ajax request will return json object
     * and for non-ajax request if deletion is successful, the browser will be redirected to the 'index' page.
     * @param string $id
     * @return mixed
    ...
    /**
     * Finds the TravelPlans model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param string $id
     * @return TravelPlans the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = TravelPlans::findOne($id)) !== null) {
            return $model;
        } else {
