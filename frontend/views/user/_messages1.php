...
<div class="col-md-12">
    <div class="col-md-3">
        <div class="text-center" style="margin-bottom:20px; padding:20px; padding-top: 50px;background-color:white; width:100%;border:solid 1px lightgrey; float:left !important">
            <?= FHtml::showCurrentLogo('70%', '') ?> <br/>

            <div style="color: cadetblue; font-size:24px; font-weight: bold; margin:30px"><?= FHtml::settingCompanyName() ?></div>
        </div>

        <a class="btn btn-sm btn-default" href="<?= FHtml::createUrl('user/inbox') ?>"><?= FHtml::t('Back to Inbox') ?> </a>
    </div>
    <div class="col-md-9" style="padding-left:30px">
        ...
        <?php if (!empty($comments)) {
            foreach ($comments as $model) {
                ...
                <div class="col-md-12">
                    <div class="col-md-1 text-center" style="padding:15px; color:grey; font-size:80%">
                        <?php if ($is_from) { ?>
                            <?= FHtml::showCurrentLogo('20px', '', 'img-circle') ?><br/>
                            <?= FHtml::showDate($model->created_date) ?>
                        <?php } ?>
                    </div>
                    ...
                    <div class="col-md-1 text-center" style="padding:15px; color:grey; font-size:80%">
                        <?php if (!$is_from) { ?>
                        <?= FHtml::showImage($image, 'app-user', '20px', '', 'img-circle') ?><br/>
                            <?= FHtml::showDate($model->created_date) ?>
        ...
        <div class="col-md-12" style="margin-top: 20px; border-top:1px dashed lightgrey; padding-top:30px">
            ...
            <div class="col-md-1 text-center" style="padding:15px">
                <?= FHtml::showImage('no_image.png', 'www', '20px', '', 'img-circle') ?>
            </div>
        </div>
